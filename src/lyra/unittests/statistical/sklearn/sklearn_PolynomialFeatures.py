...
# STATE: deanormalized_mean -> Top; df -> DataFrame; df["x_normalized"] -> Series; scaler -> PolynomialFeatures; wrong_normalized_mean -> Top; x -> NumericList
wrong_normalized_mean = df['x_normalized'].mean()
# STATE: deanormalized_mean -> Top; df -> DataFrame; df["x_normalized"] -> Series; scaler -> PolynomialFeatures; wrong_normalized_mean -> Numeric; x -> NumericList
deanormalized_mean = scaler.inverse_transform(df[['x_normalized']]).mean()
# FINAL: deanormalized_mean -> Numeric; df -> DataFrame; df["x_normalized"] -> Series; scaler -> PolynomialFeatures; wrong_normalized_mean -> Numeric; x -> NumericList
